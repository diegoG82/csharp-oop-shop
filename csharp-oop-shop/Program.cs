...
static void StampaArrayProdotti(Prodotto[] prodotti)
{

    foreach (Prodotto prod in prodotti)
    {
        Console.WriteLine($"Codice prodotto: {prod.Codice}");
        Console.WriteLine($"Nome esteso: {prod.GetNomeEsteso()}");
        Console.WriteLine($"Nome prodotto: {prod.Nome}");
        Console.WriteLine($"Descrizione del prodotto: {prod.Descrizione}");
        Console.WriteLine($"Prezzo: {prod.GetPrezzoBase()} euro");
        Console.WriteLine($"Prezzo con iva: {prod.GetPrezzoIvato()} euro");
        Console.WriteLine();
    }
}
